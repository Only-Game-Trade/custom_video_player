...
+ (FLTPausePointsMessage *)fromList:(NSArray *)list;
+ (nullable FLTPausePointsMessage *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end
...
+ (instancetype)makeWithTextureId:(NSNumber *)textureId
    pausePointsInMilliseconds:(NSArray<NSNumber *> *)pausePointsInMilliseconds {
  FLTPausePointsMessage* pigeonResult = [[FLTPausePointsMessage alloc] init];
  pigeonResult.textureId = textureId;
  pigeonResult.pausePointsInMilliseconds = pausePointsInMilliseconds;
  return pigeonResult;
}
+ (FLTPausePointsMessage *)fromList:(NSArray *)list {
  FLTPausePointsMessage *pigeonResult = [[FLTPausePointsMessage alloc] init];
  pigeonResult.textureId = GetNullableObjectAtIndex(list, 0);
  NSAssert(pigeonResult.textureId != nil, @"");
  pigeonResult.pausePointsInMilliseconds = GetNullableObjectAtIndex(list, 1);
  NSAssert(pigeonResult.pausePointsInMilliseconds != nil, @"");
  return pigeonResult;
}
+ (nullable FLTPausePointsMessage *)nullableFromList:(NSArray *)list {
  return (list) ? [FLTPausePointsMessage fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    (self.textureId ?: [NSNull null]),
    (self.pausePointsInMilliseconds ?: [NSNull null]),
  ];
}
@end
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 131: 
      return [FLTPausePointsMessage fromList:[self readValue]];
    case 132: 
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FLTCreateMessage class]]) {
    ...
    [self writeByte:130];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[FLTPausePointsMessage class]]) {
    [self writeByte:131];
    [self writeValue:[value toList]];
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.clearAllPausePoints"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(clearAllPausePoints:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(clearAllPausePoints:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTTextureMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api clearAllPausePoints:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
