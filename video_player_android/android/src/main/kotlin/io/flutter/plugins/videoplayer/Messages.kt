...
data class AutoPauseTriggeredEvent (
...
) : PlatformVideoEvent()
 {
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is AutoPauseTriggeredEvent) {
      return false
    }
    if (this === other) {
      return true
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      135.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          AutoPauseTriggeredEvent.fromList(it)
        }
      }
      136.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
...
interface VideoPlayerInstanceApi {
   ...
   */
  fun setPausePoints(pausePointsInMilliseconds: List<Long>)
  /** Removes all scheduled pause points. */
  fun clearAllPausePoints()
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: VideoPlayerInstanceApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.setPausePoints$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pausePointsInMillisecondsArg = args[0] as List<Long>
            val wrapped: List<Any?> = try {
              api.setPausePoints(pausePointsInMillisecondsArg)
              listOf(null)
            } catch (exception: Throwable) {
              MessagesPigeonUtils.wrapError(exception)
            }
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.video_player_android.VideoPlayerInstanceApi.clearAllPausePoints$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              api.clearAllPausePoints()
              listOf(null)
            } catch (exception: Throwable) {
              MessagesPigeonUtils.wrapError(exception)
            }
